...
写作理论（上下文指针、信息层级、完成判据、正面表述、修剪纪律）见
...
**description 是唯一的触发机制**——它是一个常驻上下文的**上下文指针**，
agent 只靠它在对话中决定是否加载本技能。写作要点：

- 同时包含「做什么」和「何时用」，把所有触发线索放这里，不放正文
- **触发词前置**；一个分支配一个触发词，同义改写合并成一个，只留
  真正不同的分支
- 适当"外向"一点：用户不会总用你预想的措辞。除了精确场景，也覆盖
  邻近说法（中英文触发词都列上，如「总结/精读/summarize」）
- 写完自查：每个词都在做触发工作吗？正文已能自明的信息不进 description
...
两种方式的取舍是**两种负载的交换**：默认触发让 description 常驻
上下文，花**上下文负载**（agent 窗口的 token）换可发现性；显式调用
零上下文负载，但**人要记得它存在**（认知负载）。只有当 agent 必须
自己到达这个技能、或其他技能需要引用它时才选默认触发；只会手动
触发的就让它显式。
...
- **渐进式披露**：SKILL.md 控制在 ~500 行内。装不下的内容放
  ...
  披露的判据是**分支**：每个分支都需要的内联，只有部分分支才碰到的
  推到指针后面
...
- **祈使句**、解释**为什么**而不是堆 MUST；想写 ALWAYS/NEVER 时，
  先想想能不能把理由讲清楚来替代。**用正面表述**：写「写单行注释」
  而非「不要写长注释」，禁令只留给无法正面表述的硬护栏
- **每个步骤以清晰的完成判据收尾**：agent 能否分辨 done 和 not-done？
  模糊判据（「达成共识」）诱发过早完成；「每条改动都被覆盖到」这类
  既可检验又求完备的判据最有效
- **可复用的重复劳动进 scripts/**：测试中如果每轮都要手写同样的
  辅助脚本，就固化一份放进 `scripts/`
- **修剪纪律**：逐句查 no-op——模型默认就会做的指令，付了负载什么
